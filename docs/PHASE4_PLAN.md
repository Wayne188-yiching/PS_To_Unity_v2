...
> **接手說明**：這份是 Phase 4 grill-me 規劃對話的決策摘要。任何 Claude session（不論帳號 / 機器）讀完這份，就能直接從 **Q6（CanvasGroup 觸發條件）** 續上 grilling。
> 更新時間：2026-06-18（grill-me 進度完成至 Q5，下次從 Q6 起）。
...
`[GRID]` 觸發後，GridLayoutGroup 各欄位規則：
...
**Q5.1 / Q5.2 / Q5.3 邊界條件決議：**

| 子題 | 決議 | Why |
|---|---|---|
| Q5.1 `constraint` | **FixedColumnCount**，欄數 = 第一行同 Y 節點數 | Flexible 在 runtime 因 viewport 寬度自動換行，跟 PS 看到的版面對不上 |
| Q5.2 不等大子節點（寬高差異 > 20%） | **JSX 報 warning + 降級成普通 group**（不掛 GridLayoutGroup） | GridLayoutGroup 強制等大 cell；硬塞視覺會爆掉；降級至少不破壞排版 |
...
> 我要繼續 Phase 4 規劃。請先讀 `docs/PHASE4_PLAN.md`，然後從 **Q6（CanvasGroup 觸發條件）** 開始 grill 我（沿用 /grill-me + /karpathy-guidelines 流程）。Q1–Q5 都已決議完，不要重問。
