...
def build_psd_agent(request: PipelineRequest, tools: CaseTools | None = None, *, planning_only: bool = False) -> Agent:
    ...
    return Agent(
        ...
        instructions=(
            ...
            "using only stable IDs from visibleLayerIndex. Create parent groups before child groups. List sibling move actions in descending "
            "siblingIndex order, bottom-most layer first, so Photoshop stacking order is preserved. Never emit a rename whose new_name already equals "
            "the layer's current name. Every move names exactly one parent: parent_layer_id for a group that already exists, or parent_ref for a "
            "group this plan creates earlier; setting both or neither is rejected. A create_group ref is a lowercase ASCII identifier that is unique "
            "within the plan, and parent_ref may only name a ref created earlier in the same plan. A proposal does not require prior approval: when confirmedSemantics makes only part of the restructure "
