...
垂直翻轉使差異惡化至 89.56，無翻轉；專案為 Linear 色彩空間，但線性↔sRGB 三種轉換組合均使差異惡化（76.78／61.32／65.99），非 gamma 問題。

### 更正：版面並未對齊

本文件初版以「−3..+3 全域位移搜尋的最佳解為 (0,0)」推論無位移，並據此結論「排版與素材正確」。**該結論錯誤，已撤回。**

該搜尋有兩個缺陷：範圍僅 ±3px，而實際偏移為 4px 與 10px，全部落在範圍外；且只檢驗整張圖的剛性平移，對逐元件各自不同的偏移完全無效。改以 220×220 區塊、±10px 的逐塊搜尋重測後，結果如下：

| 區域 | 最佳位移 (dx, dy) | 平均色差 | 降低 |
|---|---|---|---|
| 第 1 列內容 | −4, −4 | 38.10 → 16.76 | 56% |
| 內容區多數區塊 | −4, −4 | 26.65 → 5.37 | 80% |
| x≈1320 一帶 | **−10, −4** | 35.92 → 16.48 | 54% |
| 標題文字 | **0, +23** | 50.61 → 40.09 | 21% |
| 面板外框左上角 | 0, 0 | 22.87 | 0% |
| 右側捲軸 | 0, 0 | 20.16 | 0% |
| 底部結算列 | +1, 0 | 25.96 → 24.15 | 7% |

面板外框、捲軸與底部結算列確實對齊，但內容區普遍往左上偏 4px、局部達 10px，標題文字則下移 23px。**偏移量因元件而異，不是單一平移可解釋**，屬真實的非剛性跑版。

Validator 的 129/129 與此不衝突：它比對的是 Unity 節點與 `layout.json`，兩者同源於 PSD，只能證明「Unity 照著匯出的版面生成」，無法證明「匯出的版面等於 Photoshop 畫面」。初版把前者當成後者的證據，是推論上的錯誤。

殘差的另一部分確實來自字重（專案已將 MiSans 系列整併為單一字型資產）與邊緣抗鋸齒，但這不足以解釋上述位移。

此門檻改判為**未通過**：逐像素對照已執行並量化，結果顯示版面尚未對齊。後續需釐清偏移來源——候選包含 exporter 對帶圖層效果之節點的 rect 計算、importer 的座標換算，以及本次截圖用的渲染治具（治具只會造成整體一致的偏移，無法解釋逐元件差異）。
...
因此目前可宣告「Hall_Ranking 的 Photoshop 匯出閉環 PASS、Unity deterministic 結構閉環 129/129 相符且 Pipeline Validator PASS、reskin 決策無待決項，且修正兩個會造成靜默資料遺失的 exporter 缺陷」。

**不可宣告視覺驗收結案，且已知未通過**：逐塊實測顯示生成畫面與設計稿存在逐元件的非剛性位移（內容區 4px、局部 10px、標題 23px），須先釐清並修正；Pointer drag／Elastic rebound 則因素材不含 scroll 群組而未驗。`liveAcceptance` 依設計維持 `NOT_RUN`——Pipeline Validator 的 PASS 只覆蓋 IR 到 Prefab，不等同實機視覺與操作驗收，本次正是此差距的實例。
