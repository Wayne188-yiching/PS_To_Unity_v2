...
- Since v2.14.0, Prefab generation creates or updates one atlas per language: `{Module}_Atlas.spriteatlasv2` for Base and `_CHS`, `_CHT`, or `_EN` atlases only when those folders contain images. Each atlas has exactly one packable — its matching language folder — and is packed once after all TextureImporters are ready. Atlas max size is selected from 2048/4096/8192 according to the largest source image. Existing projects may still contain the legacy `SpriteAtlas.spriteatlasv2`; remove it manually after verifying the new atlases to avoid packing the same images twice.
- Pixel dedup still redirects generated `Image` references to canonical Sprites. When the importer copied images into Unity, redundant alias PNGs are removed from the destination only after references are redirected; the source package stays intact, so the same layout JSON remains re-importable. When the source already lives inside Unity `Assets`, the importer does not delete source files and instead reports duplicate aliases in the Console. Images added directly to Base/CHS/CHT/EN are automatically included by that language folder's packable.
